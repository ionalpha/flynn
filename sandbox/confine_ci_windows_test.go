...
// TestOutboundReachableUnconfinedOnCI guards against a hollow green run of the Windows
...
func TestOutboundReachableUnconfinedOnCI(t *testing.T) {
	...
	res, err := open.Exec(context.Background(), Command{Line: `curl --max-time 6 -s -o NUL http://1.1.1.1`})
	if err != nil {
		t.Fatalf("exec: %v", err)
	}
	if res.ExitCode != 0 {
