...
// FuzzDecodeArgv proves the decoder is tolerant of arbitrary, possibly malformed input
// from the control variable: it must return an error or a result, never panic. The
// launcher treats any decode error as a refusal to run, so that tolerance is what
