...
the spine. Time, identity, and randomness come only from injected boundaries
...
| Layer | Packages | Role |
|-------|----------|------|
| L0 foundation | `clock`, `fault`, `llm`, `observe` | Pure boundaries: time source, error taxonomy, the model port, the logging/tracing port. No internal imports. |
...
## Ports (the boundaries that keep it swappable)

A port is an interface the engine depends on so the thing behind it can change
without touching the engine. The critical ones:
...
- **`spine.Log`** is the append-only event store: monotonic `Seq` per stream,
  ...
  checkpoints) so a rebuild stays bounded as a stream grows. This is the foundation
...
- **`clock`**, **`ids`**, **`hlc`** the determinism boundaries; **`fault`** the error
