...
func TestReplayEquivalence(t *testing.T) {
	...
	// generated IDs, so the comparison is exercising the boundaries, not empty logs.
	if len(first) == 0 {
		t.Fatal("no events on the spine; the scenario did not exercise the determinism boundaries")
	}
}
