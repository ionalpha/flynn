...
func TestRunWritesFileThroughSandbox(t *testing.T) {
	...
	result, err := runLearningMission(ctx, &out, model, harness.Plan{}, nil, dir, "create hello.txt with a greeting", memStore(t), nil, false)
	if err != nil {
		t.Fatalf("run: %v", err)
	}
...
func TestRunRejectsSandboxEscape(t *testing.T) {
	...
	if _, err := runLearningMission(ctx, &out, model, harness.Plan{}, nil, dir, "try to escape", memStore(t), nil, false); err != nil {
		t.Fatalf("run: %v", err)
	}
...
func TestRunRemembersAcrossRuns(t *testing.T) {
	...
	if _, err := runLearningMission(ctx, &out, run1, harness.Plan{}, distiller, dir, "set up the project", store, nil, false); err != nil {
	...
	if _, err := runLearningMission(ctx, &out, run2, harness.Plan{}, nil, dir, "install deps with pnpm", store, nil, false); err != nil {
...
func TestRunFeedsTranscriptToDistiller(t *testing.T) {
	...
	if _, err := runLearningMission(ctx, &out, model, harness.Plan{}, rec, dir, "write x.txt", memStore(t), nil, false); err != nil {
		t.Fatalf("run: %v", err)
	}
...
func TestRunReinforcesRecalledSkill(t *testing.T) {
	...
	if _, err := runLearningMission(ctx, &out, model, harness.Plan{}, &fakeDistiller{}, dir, "deploy with docker", store, nil, false); err != nil {
		t.Fatalf("run: %v", err)
	}
...
func TestRunVerifiesCapturedSkill(t *testing.T) {
	...
	if _, err := runLearningMission(ctx, &out, model, harness.Plan{}, distiller, dir, "do the work", store, nil, false); err != nil {
		t.Fatalf("run: %v", err)
	}
...
func TestRunSpineIsDurableAndAddressable(t *testing.T) {
	...
	if _, err := runLearningMission(ctx, &out, model, harness.Plan{}, nil, workdir, "do the thing", store1, nil, false); err != nil {
		t.Fatalf("run: %v", err)
	}
