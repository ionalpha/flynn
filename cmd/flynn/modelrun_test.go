...
func TestServeModelHappyPath(t *testing.T) {
	...
	ep, err := r.serveModel(context.Background(), localTestModel())
	if err != nil {
...
func TestServeModelRejectsHostedModel(t *testing.T) {
	r, _ := newFakeRunner(t, "")
	_, err := r.serveModel(context.Background(), catalog.ModelSpec{ID: "openai:gpt-5.5", Kind: catalog.KindAPI})
	if err == nil {
...
func TestServeModelRejectsNoDirectDownload(t *testing.T) {
	...
	_, err := r.serveModel(context.Background(), m)
...
func TestServeModelPropagatesProvisionError(t *testing.T) {
	...
	_, err := r.serveModel(context.Background(), localTestModel())
